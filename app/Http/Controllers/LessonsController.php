...
class LessonsController extends Controller
{
	...
	public function save_data_answer(Request  $request) 
	{
		$data = $request->test;
	...
	public function cmp($a, $b) {
		return strcmp($a['questionorder'], $b['questionorder']);
	}
    ...
    public function show($course_id, $lesson_slug)
    {
		...
		{
		foreach($lesson->questions as $ques) {
			$ans = DB::table('user_answer')->where('question_id',$ques->id)->where('user_id',\Auth::id())->first();
			if($ans)
			{
				$answers[$ques->id] = $ans->answer;
			}
		}
		}
	//	$allquestions =DB::table('questions')->where('course_id',$lesson->course->id)->get();
		...
		{
			//$tmparr = $newquestionadjust->toArray();
			usort($tmparr, function($a, $b) {return $a['questionorder'] > $b['questionorder'];});
			$newtmparr =  (object) $tmparr;
			array_push($pagewithquestions,$newtmparr);
		}
		 
		
		return view($this->path . '.courses.lesson', compact('lesson','pagewithquestions','answers', 'previous_lesson', 'next_lesson', 'test_result',
