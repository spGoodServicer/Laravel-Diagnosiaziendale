...
@if(isset($content->type)==1)
    <div class="mb-2 radiogroup cursorbar"><div class="d-flex">
            ...
            <div class="range-slider col-10">
              ...
              <input name="b_range" class="" id="range" data-id="{{$question->id}}" type="range" min="{{$content->min_value}}" step="{{$content->steps}}" max="{{$content->max_value}}" value="{{(isset($answers[$question->id])) ?  $answers[$question->id] : $content->min_value}}" data-sym="€">
...
<script>
$('input[name="b_range"]').click(function(){
	storeFun($(this).data('id'),$(this).val());
});
</script>
@endif