...
@php
	$i = 0;
@endphp
@foreach ($pagewithquestions as $key=>$questionArr)
    <div class="pages" id="page{{$key}}" style="display:{{($i > 0) ? 'none' : ''}}">
	@php
		$i++;
	@endphp
            ...
            @if($question->titlelocation == 'col-12' && $question->answerposition == 'col-12' && $question->imageposition == 'col-12')
                ...
                <div class="row">
                    ...
                    <div class="{{$question->imageposition}}">
                        <img src="{{$question->questionimage}}" width="{{$question->imagewidth}}">
                    </div>
                    @endif
                    <div class="{{$aligment}}">
...
<div class="text-right">
    <div id="navigation-btns" class="mb-2" >
        <button type="button" id="pre-pg" class="btn btn-danger">Previous</button>
        @php
			$j = 0;
		@endphp
		@foreach ($pagewithquestions as $key=>$questionArr)
			<button type="button" id="btnpage{{$key}}"  class="btn {{($j == 0) ? 'btn-default' : 'btn-danger'}} pagechange" key="page{{$key}}">{{$j+1}}</button>
			@php
				$j++;
			@endphp
		@endforeach
...
<script>
    ...
    $('#next-pg').click(function(){
		...
		$('.pagechange').each(function(){
			if($(this).hasClass('btn-default'))
			{
				if($(this).next('.pagechange').attr('key') != undefined)
				{
				$(this).removeClass('btn-default').addClass('btn-danger');
				console.log($(this).next('.pagechange').attr('key'));
				
				nextpageid = $(this).next('.pagechange').attr('key');
				}
			}
		});
		...
		{
			$('.pages').hide();
			$('#btn'+nextpageid).removeClass('btn-danger').addClass('btn-default');
			$('#'+nextpageid).show();
			nextpageid = '';			
		}
	...
	$('#pre-pg').click(function(){
		...
		$('.pagechange').each(function(){
			if($(this).hasClass('btn-default'))
			{
				if($(this).prev('.pagechange').attr('key') != undefined)
				{
					$(this).removeClass('btn-default').addClass('btn-danger');
					nextpageid = $(this).prev('.pagechange').attr('key');
				}
			}
		});
		if(nextpageid != '' && nextpageid != 'undefined')
